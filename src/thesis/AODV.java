  ...
  public static final int TTL_START = 5;
    ...
    public void newNarrativeMessage(String srcID, String destID,
        ...
        if (!this.att.id.equals(srcID)) {
            OutputHandler.dispatch(EventManager.outError(this.att.id
        ...
        if (RouteTable.containsKey(destID)) {
            ...
            /**
             * The destination is in our RouteTable. Make sure that the route is
             * valid and not too old.
             */
            if ((DestEntry.getState() == RouteEntry.StateFlags.VALID)
                ...
                OutputHandler.dispatch(EventManager.outDebug(MsgStr));
                /**
                 ...
                 *
                 * During this time the message will need to be placed on the
                 * WaitQueue.
                 */
                sendRREQ(MsgDestID);
            ...
            /**
             * Do not have a route to the destination node. Need to perform a
             * Route Request.
             */
            sendRREQ(MsgDestID);
            /**
             * Add the message into the wait queue. Already did the work to
             * build the message so we use the message string. We only need the
             * destination ID for the next iteration to figure out of we have a
             * good route to send to.
             */
            addMessageToWaitQueue(MsgOrigID, MsgDestID, MsgStr);

            OutputHandler.dispatch(EventManager.outDebug(this.att.id
   ...
   *          Message to be transmitted.
    ...
    private void sendMessage(Message message) {
        ...
        if (!this.att.isPromiscuous) {
             ...
             *
             * Message type is always the first token in the message string.
             * Split on the '|' and get the first item in the resultant Array of
             * Strings.
             */
            ...
            if (MsgType.equals("NARR")) {
                OutputHandler.dispatch(EventManager.outNarrMsgTransmitted(this.att.id, message));
                return;
            }
    ...
    private void receiveMessage(Message message) {
        ...
        if (MsgType.equals("RREQ")) {
            receiveRREQ(message);
            OutputHandler.dispatch(EventManager.outControlMsgReceived(this.att.id, message));
            return;
        }

        if (MsgType.equals("NARR")) {
            receiveNarrative(message);
            OutputHandler.dispatch(EventManager.outNarrMsgReceived(this.att.id, message));
            return;
        }
     ...
     *
     * @param DestNodeID The Node ID for the destination that a route is needed
     * for.
     */
    void sendRREQ(String DestNodeID) {
         ...
         * known sequence number for the destination and also if the source node
         * has already requested a route for the destination.
         */
        ...
        if (DestRouteEntry == null) {
            /**
             * Create a new Route Entry in the Route Table to record that a RREQ
             * has been sent.
             */
        ...
        if ((DestRouteEntry.getState() == StateFlags.RREQSENT)
            ...
            if (DestRouteEntry.getLifetime() > CurrentTick) {
                /**
                 * A RREQ has already been sent for this node but a response has
                 * not yet been received. Do not send another RREQ.
                 */
                return;
            } else {
                /**
         ...
         * table. If the destination sequence number == 0 and set the
         * destination sequence unknown flag(U).
         */
     ...
     *
     * @param message The Route Request message that was received.
     */
     ...
     * and Forward on.
        ...
        /**
         * Split Message into fields based on '|' delimiters and store in
         * MsgArray.
         */
        MsgArray = message.message.split("\\|");

        /**
         * Store message fields into local variables. Yes this is not really
         * needed but I (SAK) think it makes the code more readable.
         */
        MsgType = MsgArray[0];
        MsgFlags = MsgArray[1];
         ...
         *
         * This happens when a neighbor node re-broadcasts the RREQ and it
         * creates a rather interesting loop.
         */
        ...
        if (this.RREQHistory.containsKey(MsgSrcID)) {
            if (this.RREQHistory.get(MsgSrcID) >= MsgRREQID) {
                ...
                /**
                 * The RREQ is newer than this node has seen. Add it to the
                 * history and continue to process it.
                 */
        ...
        /**
         * If this node does not have a Route to the originator of the RREQ add
         * the Originator to the Route Table otherwise update the Originators
         * Route Entry.
         */
         ...
         */
        /**
         * If our route is not VALID take all of the message's info. This may be
         * a little over simplified from the RFC, but Meh. For now.
         */
        if (DestEntry.getState() != RouteEntry.StateFlags.VALID) {
        ...
        /**
         * If this node is the desired Destination Node we can service this
         * RREQ.
         */
        ...
        /**
         * Check to see if the desired Destination ID is in this nodes Route
         * Table.
         */
        if (RouteTable.containsKey(MsgDestID)) {
            /**
             * This node has knowledge of the desired destination node, but
             * still need to check more parameters before sending a RREP.
             *
            ...
            /**
             * If the Route Entry is marked valid and the TTL is not passed then
             * send a RREP. Otherwise continue on and forward the RREQ. If all
             * goes well this node will get updated an updated route when it
             * forwards back the RREP.
             */
            if ((DestEntry.getState() == RouteEntry.StateFlags.VALID)
                    && (DestEntry.getLifetime() >= this.CurrentTick)) {
                sendRREP(MsgDestID, MsgSrcID, message.originId);
        ...
        /**
         * This node can not service this RREQ directly. Forward on the RREQ
         * into the network.
         *
     ...
     *
     * @param DestNodeID The Destination Node ID for the node that is no longer
     * reachable.
     */
    void sendRERR(String DestNodeID) {
        ...
        /**
         * Iterator and RouteEntry for going through the values in the
         * RouteTable.
         */
        ...
        /**
         * Invalidate Route Entry in the Route Table and increase lifetime by
         * delete period.
         */
        ...
        /**
         * Get Iterator for RouteTable and then traverse it looking for nodes
         * that have DestNodeID as their next hop.
         */
        RouteTableIter = RouteTable.values().iterator();

        /**
         * Check each route entry in the route table to see if the next hop is
         * the error destination ID. If it then mark the Entry as Reparable. The
         * Route is marked Repairable because it is no longer valid but there
         * may be another path the the destination node for that route through a
         * new next hop.
         *
     ...
     *
     *
     * @param message The Route Error Message that was received.
     */
    void receiveRERR(Message message) {
        ...
        /**
         * Route Table Entry used to add and modify the entries in the Route
         * Table.
         */
        RouteEntry DestEntry;
     ...
     *
     *
     * @param DestNodeID The node that the RREQ was made for.
     * @param RequesterID The node that originated the RREQ.
     * @param SenderID The node that this node received the RREQ from.
     */
    void sendRREP(String DestNodeID, String RequesterID, String SenderID) {
        ...
        /**
         * If this node is the desired Destination Node just send the reply no
         * need to update a Precursors list.
         */
        if (this.att.id.equals(MsgDestID)) {
            ...
            /**
             * LifeTime should be the normal Route LifeTime. This is different
             * if below when we look up an actual entry in the Route Table.
             *
             * Note that this value is given in 'Ticks' but is an interval and
             * does not have a reference to this nodes current tick count.
             */
        ...
        /**
         * Find the Destination Node in our Route Table and reply with our
         * Route.
         */
        if (!this.RouteTable.containsKey(DestNodeID)) {
            ...
            /**
             * This node can not send a RREP for the Destination needed so drop
             * the message.
             */
            return;
        }
         ...
         * RREP.
         *
         * MsgOrigID = The Original Creator of the RREQ. SenderID = The Node
         * that sent us the RREQ.
         */
         ...
         *
         * Must convert Lifetime into an interval instead of being node
         * dependent.
         */
    ...
    void receiveRREP(Message message) {
         ...
         */
        /**
         * Message object that will be passed to sendMessage. For forwarding on
         * the RREP.
         */
        Message Msg;
        /**
        ...
        MsgFlags = MsgArray[1];
        /**
         * Add 1 to the Hop Count for the hop that it took for the message to
         * get to this node.
         */
         ...
         *
         * If Destination ID and Original ID are Equal then this is a Hello
         * Message.
         */
        if (MsgDestID.equals(MsgOrigID)) {
            /**
             * Check to see if the node sending the hello message is in our
             * Route Table.
             */
            if (this.RouteTable.containsKey(MsgDestID)) {
                ...
                if (DestEntry.getSeqNum() <= MsgDestSeqNum) {
                    ...
                    OutputHandler.dispatch(EventManager.outDebug(this.att.id + " Updated "
                            + MsgDestID + " in its RouteTable"));
                }

            } else {
                /**
                 * The sending node is NOT in the receiving node's Route Table.
                 * Add it.
                 *
                 * The parameters for the RouteEntry are fairly straight forward
                 * except for Lifetime. Lifetime is sent to us as an interval so
                ...
                OutputHandler.dispatch(EventManager.outDebug(this.att.id + " Added "
        ...
        if (MsgOrigID.equals(this.att.id)) {
            if (this.RouteTable.containsKey(MsgDestID)) {
                ...
                /**
                 * The RREQ that was sent out to generate this is really old.
                 * Drop the message?
                 */
                OutputHandler.dispatch(EventManager.outDebug(this.att.id
            ...
            /**
             * This node originated the RREQ that resulted in this RREP. No need
             * to forward it on so we are done processing this message.
             */
            return;
         ...
         */
        /**
         * Lookup the DestID of the RREP in our Route Table. If we do not
         * already have an entry create an entry, otherwise we have to make some
         * decisions.
         */
        if (!this.RouteTable.containsKey(MsgDestID)) {
            /**
             * Create the Destination's Route Entry with all of the information
             * from the message and put it into this node's Route Table..
             */
            DestEntry = new RouteEntry(MsgDestID, MsgDestSeqNum,
                    RouteEntry.StateFlags.VALID, MsgHopCount, message.originId,
                    this.CurrentTick + MsgLifetime);
            ...
            /**
             * Check to see if this RREP has 'better' information than what is
             * in our Route Entry. 'Better' gets rather complicated.
             */
            ...
            /**
             * If our route is not VALID take all of the message's info. This
             * may be a little over simplified from the RFC, but Meh. For now.
             */
            if (DestEntry.getState() != RouteEntry.StateFlags.VALID) {
                DestEntry.setSeqNum(MsgDestSeqNum);
                DestEntry.setState(RouteEntry.StateFlags.VALID);
             ...
             * (>=) than ours and our hop count is >= the messages then update
             * our info. Else ours is better. We will still forward the other.
             * Most likely we already replied to the original RREQ, but don't
             * change this RREP.
             */
            ...
            /**
             * Update the Route Entry's precursor's list to contain the
             * destination node that this RREP is being sent to as well as the
             * next hop of this RREP.
             *
             * MsgOrigID = The Original Creator of the RREQ. SenderID = The Node
             * that sent us the RREQ.
             */
        ...
        /**
         * Lookup the origin of original RREQ so that we can decide how to
         * forward on this RREP. If we don't have a route just Broadcast it? Not
    ...
    void sendHello() {
        ...
        /**
         * The node should only send out a Hello Message every Hello_Interval
         * ticks. If it is not time to send a new hello message yet then return
         * with out sending a message.
         */
    ...
    void receiveNarrative(Message message) {
        ...
        /**
         * Route Table Entry used to get the destination ID info in our Route
         * Table.
         */
        RouteEntry DestEntry;
        ...
        /**
         * Check to see if this node is the final destination of the message. If
         * it is great if not we need to forward it.
         */
        if (this.att.id.equals(MsgDestID)) {
            /**
             * The message has reached its final destination. Consider it
             * delivered.
             */
            OutputHandler.dispatch(EventManager.outMsgRecieved(MsgOrigID, MsgDestID,
                    MsgText));
            return;
        }
        ...
        if (RouteTable.containsKey(MsgDestID)) {
            ...
            /**
             * This node does not have a route to the desired destination and
             * thus should not have been used in the route. ERROR.
             */
            OutputHandler.dispatch(EventManager.outNodeInfo(this.att.id
    ...
    private void addMessageToWaitQueue(String srcID, String destID, String msgStr) {
         ...
         * PATH_DISCOVERY_TIME as the amount of time that the message is valid
         * for since the reason the message is normally placed onto the wait
         * queue is because the node is waiting on getting a Route Reply back
         * which can take a maximum of PATH_DISCOVERY_TIME to return.
         */
    ...
    private void processWaitQueue() {
        ...
        while (WaitQueueIter.hasNext()) {
            ...
            /**
             * Check the status of our Route Entry for the destination node of
             * the message.
             */
            if (this.RouteTable.containsKey(WaitEntry.DestinationID)) {
                ...
                if (DestEntry.getState() == RouteEntry.StateFlags.VALID) {
                    /**
                     * Have a valid route to the desired destination of this
                     * message so send it.
                     */
    ...
    private void checkRouteTable() {
        ...
        while (RouteTableIter.hasNext()) {
            ...
            if (TempRouteEntry.getLifetime() <= this.CurrentTick) {
                /**
                 * The lifetime has expired, so based on the current State of
                 * the route determine the next action.
                 */

                /**
                 * For Routes marked as VALID, Expire them. All other states are
                 * a result of this node being in a waiting position for this
                 * route. The outside stimulus has not occurred with in the time
                 * out so remove the route entry.
                 */
    ...
    public void clockTick() {
        ...
        receiving = false;
            ...
            check=true;
            setEnergy(0);
            OutputHandler.dispatch(EventManager.outDeleteNode(this.att.id));
            //InputHandler.dispatch(EventManager.inDeleteNode(this.att.id));
            return;
        } else 
            return;
    }
     ...
     * Route Request ID
     *
     * The RREQ ID must be unique to each route request sent out by a node. It
     * is incremented immediately before a route request is generated.
     */
     ...
     *
     * The node sequence number is a identifier that is unique across all
     * protocol control messages(RREQ, RREP, RERR) for a node. It is incremented
     ...
     * Wait Queue
     *
     * Queue of message that can not be sent yet. Most likely waiting for a
     * RREP.
     */
     ...
     *
     * This hash map is used to track the last known sequence number request for
     * a given Destination ID.
     *
     ...
     * getNodeDialog
     *
     * This method will construct a JDialog from select node information to
     * return back to the GUI to be displayed.
     *
     * @param void
     *
     * @return JDialog
     ...
     * updateNodeDialog
     *
     * This method will update an already constructed JDialog box that is
     * already being displayed by the GUI.
     *
     * @param JDialog
     *
     * @return void
